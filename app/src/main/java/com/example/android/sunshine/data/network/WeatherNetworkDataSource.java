...
import com.example.android.sunshine.data.POJO.ListItem;
...
public class WeatherNetworkDataSource {
    ...
    public static final String location = "Mountain View, CA";
    ...
    void fetchWeather() {
        ...
        mExecutors.networkIO().execute(() -> {
            ...
            Call<Response> call = webService.repos
                    (location, format, units, Integer.toString(WeatherNetworkDataSource.NUM_DAYS));
            call.enqueue(new Callback<Response>() {
                @Override
                public void onResponse(Call<Response> call, retrofit2.Response<Response> response) {
                    Object[] weatherArray = response.body().getList().toArray();
                    ...
                    for (int i = 0; i < weatherArray.length; i++) {
                        ListItem listItem = response.body().getList().get(i);
                        int id = response.body().getList().get(i).getWeather().get(0).getId();
                        long dateTimeMillis = normalizedUtcStartDay + SunshineDateUtils.DAY_IN_MILLIS * i;



                        WeatherEntry weather = new WeatherEntry(
                                id, new Date(dateTimeMillis), listItem.getTemp().getMax(), listItem.getTemp().getMin(),
                                listItem.getHumidity(), listItem.getPressure(), listItem.getSpeed(),
                                listItem.getDeg());
                        weatherEntries[i] = weather;
                        WeatherResponse weatherResponse = new WeatherResponse(weatherEntries);
                        if (weatherResponse != null && weatherResponse.getWeatherForecast().length != 0) {
                            mDownloadedWeatherForecasts.postValue(weatherResponse.getWeatherForecast());
                        }
                    }
                }
                @Override
